...
class Corpus:
    ...
    def append_all_arguments(self, proposal_id: int, arguments: List[EvaluatedArgument]) -> None:
        """Append a list of arguments to the corpus of arguments"""
        for argument in arguments:
            self._append_argument(proposal_id, argument)
    
    def filter_by_completed(self, field_name: str, values) -> Self:
        ...
        for proposal_id in self.arguments_dict.keys():
            ...
            arguments = [argument for argument in arguments if argument.get_evaluation().get_field(field_name) in values]
    ...
    def save_to_file(self, filename: str, fields: List[str] = ['claim', 'claim_premise', 'aspect', 'premise_validation', 'coherence', 'consistence', 'persuasion', 'emotional_ethic']) -> None:
        """Save the corpus of arguments in a file with the specified fields"""
        with open(filename, 'w', encoding = 'utf-8') as file:
            file.write(self.format_file(fields))

    
    def filter_by(self, field_name: str, values) -> Self:
...
def classify_arguments_or_not_with_aspect() -> Corpus:
    """Classify the arguments as arguments or not arguments based on the value of the fields 'Claim+premise?WHY?' and 'Relacionado con aspecto?' of the evaluation of the arguments"""
    corpus = create_complete_corpus()
    filtered_corpus = corpus.filter_by_completed('claim_premise', 1).filter_by_completed('aspect', 1)
    return filtered_corpus
...
def main():
    ...
    corpus = create_complete_corpus()
    corpus = corpus.filter_by_completed('premise_validation', 1).filter_by_completed('coherence', [1, 2])
    corpus.save_to_file('complete_corpus_filtered.txt', ['claim', 'claim_premise', 'premise_validation', 'coherence'])
